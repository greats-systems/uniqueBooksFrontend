...
import { getPrintQueue } from 'store/slices/printQueue';
...
const Books = () => {
    ...
    const { printQueue } = useSelector((state) => state.printQueue);
    React.useEffect(() => {
        // dispatch(getCustomers());
        dispatch(getPrintQueue());
    }, [dispatch]);
    React.useEffect(() => {
        // setRows(customers);
        setRows(printQueue);
    ...
    const handleSearch = (event) => {
        ...
        if (newString) {
            const newRows = rows.filter((row) => {
                ...
                const properties = ['title', 'category', 'size', 'pages', 'price', 'created_at', 'queue_status'];
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell align="center">
                                            {row.queue_status === 'queued' && <Chip label="Queued" size="small" chipcolor="success" />}
                                            {row.queue_status === 2 && <Chip label="InActive" size="small" chipcolor="orange" />}
